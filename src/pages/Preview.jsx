...
import { useLoading, useBookmark } from "../hooks";
import { RatingStar, LoadingSpinner } from "../components/ui";
import { getRecipe, NUTRITION } from "../components/utils";
...
const Preview = () => {
  ...
  const isLoading = useLoading(2.5, bookmark.isSaved);
  ...
  return (
    <div className="w-[62.5rem] max-w-[95%] mx-auto">
      <div className="w-full max-w-full p-[1rem]" />
      {isLoading ? (
        <LoadingSpinner />
      ) : (
        <article className="md:px-[1.25rem] px-[0.5rem] md:grid lg:grid-areas-previews md:grid-areas-previews-1 md:grid-cols-previews  h-fit gap-4">
          <div className="grid-in-header">
            ...
            <p className="my-4 text-lg leading-[1.75rem]">
              These almond flour pancakes are a wonderful substitute for regular pancakes when
              watching your carbs! They are very filling and are on the dense side. This is a basic
              recipe that can be tweaked. Any sweetener can be substituted for maple syrup. Enjoy!
            </p>
                ...
                <div className="flex font-bold">
                  <div className="w-[7rem] flex justify-center h-[3.5rem] bg-[#d54215] hover:bg-[#b53811] rounded-[0.625rem] rounded-r-none">
                    ...
                    >
                      ...
                      {bookmark.isSaved ? (
                        <AiFillHeart className="text-xl fill-white" />
                      ) : (
                        <AiOutlineHeart className="text-xl fill-white" />
                      )}
                    </button>
                  </div>
          ...
          <div className="grid-in-content my-[1.5rem]">
            <div className="w-full my-[30px] border-[1px] border-solid border-[#ccc] h-fit">
              <div>
                <div className="flex items-center justify-center h-[500px] bg-[#f0f0f0]">
                  <img src={recipe.image_url} alt={recipe.title} className="object-fill" />
                </div>
              </div>
            </div>
            <div className="w-full max-w-full p-[1rem]" />
                  ...
                  {ingredients.map((ingredient, i) => (
                    <Item key={ingredient.description + i}>
                      <p className="">
                        <span
                          title="quantity"
                          className={`${ingredient.quantity ? "mr-[5px]" : ""}`}
                        >
                  ...
                  <div className="m-0">
                    ...
                    <div className="mx-auto mt-6 text-center transition-all">
                      <div className="text-[14px] px-4 leading-5">
                        Oops! We cannot find any ingredients on sale near you. Do we have the
                        correct zip code?
                      </div>
                    </div>
                  </div>
                          ...
                          <ul className="text-base border-collapse">
                            {NUTRITION.map((item) => (
                              ...
                              >
                                ...
                                <p className="inline-block w-[25%] text-right py-2">
                                  {item.percent}
                                </p>
                              </li>
                            ))}
